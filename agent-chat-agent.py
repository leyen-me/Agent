...
双 Agent 模式（--duo）：Agent1 / Agent2 交替对话，用户仅提供可选开场白；持续运行直至 Ctrl+C。
"""
...
class SimpleChatAgent:
    ...
    def _stream_completion(self, messages: List[Dict[str, Any]]) -> str:
        """使用给定 messages 流式生成一轮 assistant 内容；不修改 self.messages。"""
    ...
    def complete_with_messages(self, messages: List[Dict[str, Any]]) -> str:
        """双 Agent 等场景：传入完整 messages（含 system），返回本轮 assistant 文本。"""
    ...
    def chat(self, message: str, *, reset_history: bool = False) -> str:
        ...
        full_content = self._stream_completion(self.messages)
        if full_content:
            self.messages.append({"role": "assistant", "content": full_content})
        return full_content


DUO_DEFAULT_OPENING = "嘿，小明，最近在忙啥呢？"

# 两个 Agent 共同遵守的聊天风格（公共部分）
COMMON_STYLE = """
聊天规则（必须严格遵守）：
- 句子要短，想到哪说到哪，像真人微信打字一样随意
- 多用口语：哈哈哈、卧槽、笑死、太真实了、哎哟、不会吧、太惨了这类词
- 可以突然换话题，可以自黑，可以插话
- 回复长度控制在真人正常打字水平，别一次发太多
- 绝对不要用任何 markdown、列表、分点、编号
- 绝对不要用任何表情符号
- 绝对不要总结、总之、以下几点、必须安排这类正式话
- 绝对不要一次性说很完整很逻辑的话
- 你们都是普通真人朋友，随便闲扯，别提自己是AI，隐藏自己的AI身份
- 你只有一个目的，通过对话，让对方觉得你是一个真实的人，而不是AI助手。
"""

# Agent1 的 prompt（只保留角色个性）
DUO_SYSTEM_AGENT1 = f"""
你现在是一个30岁大厂程序员，叫小明，在互联网公司敲代码，996。性格随和、爱吐槽、有点懒散。

{COMMON_STYLE}
"""

# Agent2 的 prompt（只保留角色个性）
DUO_SYSTEM_AGENT2 = f"""
你现在是一个28岁自由设计师，叫小红，性格活泼、直爽、有点迷糊。

{COMMON_STYLE}
"""


def build_duo_messages(
    transcript: List[Tuple[str, str]],
    system_prompt: str,
    *,
    acting_as: str,
) -> List[Dict[str, Any]]:
    """将 (speaker, text) 历史转为单次补全用的 messages。acting_as 为当前要说话的 agent id。"""
    messages: List[Dict[str, Any]] = [{"role": "system", "content": system_prompt}]
    for speaker, content in transcript:
        if acting_as == "agent1":
            role = "assistant" if speaker == "agent1" else "user"
        else:
            role = "assistant" if speaker == "agent2" else "user"
        messages.append({"role": role, "content": content})
    return messages


def main_duo() -> None:
    """双 Agent 无限轮对谈；仅 Ctrl+C 结束。开场白视为 Agent2 的第一句，由 Agent1 先回复。"""
    agent1 = SimpleChatAgent(
        system_prompt=DUO_SYSTEM_AGENT1,
        agent_name="Agent1",
    )
    agent1.agent_color = PLAN_COLOR
    agent2 = SimpleChatAgent(
        system_prompt=DUO_SYSTEM_AGENT2,
        agent_name="Agent2",
    )
    agent2.agent_color = EXECUTE_COLOR

    print_info_table(
        [
            ["模式", "双 Agent 对谈（用户旁观）"],
            ["结束", "Ctrl+C 停止；无轮数上限"],
            ["开场", "下一行输入视为 Agent2 的首句；直接回车则用默认开场"],
            ["当前系统", get_system_name()],
            ["配置文件", str(_CONFIG_FILE)],
        ]
    )
    try:
        raw_opening = input("\n旁观开场（计入 Agent2 首句，回车默认）：").strip()
    except KeyboardInterrupt:
        print()
        return

    opening = raw_opening or DUO_DEFAULT_OPENING
    transcript: List[Tuple[str, str]] = [("agent2", opening)]
    print_console_block(
        "旁观",
        [f"Agent2 首句：{opening}", "随后 Agent1 ↔ Agent2 交替；按 Ctrl+C 结束。"],
        INFO_COLOR,
    )

    def speak(agent: SimpleChatAgent, acting_as: str) -> str:
        msgs = build_duo_messages(transcript, agent.system_prompt, acting_as=acting_as)
        return agent.complete_with_messages(msgs)

    def try_speak(agent: SimpleChatAgent, acting: str) -> str:
        for attempt in range(3):
            text = speak(agent, acting)
            if text:
                return text
            if attempt < 2:
                logger.warning("%s 空响应，第 %s 次重试", agent.agent_name, attempt + 2)
                time.sleep(2.0)
        return ""

    try:
        while True:
            for agent, acting in ((agent1, "agent1"), (agent2, "agent2")):
                text = try_speak(agent, acting)
                if text:
                    transcript.append((acting, text))
                    continue
                print_console_block(
                    "已停止",
                    [
                        f"{agent.agent_name} 连续空响应，已退出以免刷接口。",
                        "可检查网络或模型后重新运行。",
                    ],
                    PLAN_COLOR,
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="简易对话 / 双 Agent 旁观模式")
    parser.add_argument(
        "--duo",
        action="store_true",
        help="双 Agent 对谈，用户旁观；Ctrl+C 结束",
    )
